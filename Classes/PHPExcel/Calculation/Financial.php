...
class PHPExcel_Calculation_Financial
{
    ...
    public static function NPER($rate = 0, $pmt = 0, $pv = 0, $fv = 0, $type = 0)
    {
        ...
        if (!is_null($rate) && $rate != 0) {
            ...
            return log(($pmt * (1 + $rate * $type) / $rate - $fv) / ($pv + $pmt * (1 + $rate * $type) / $rate)) / log(
                    1 + $rate
                );
        } else {
            if ($pmt == 0) {
     ...
     * @param    float                        pmt            The payment made each period and cannot change over the
     *                                                       life of the annuity. Typically, pmt includes principal and
     *                                                       interest but no other fees or taxes.
     * @param    float                        pv            The present value - the total amount that a series of
     *                                                      future
     *                                        payments is worth now.
     * @param    float                        fv            The future value, or a cash balance you want to attain
     *                                                      after
    ...
    public static function TBILLEQ($settlement, $maturity, $discount)
    {
        ...
        if (PHPExcel_Calculation_Functions::getCompatibilityMode(
            ...
            $daysBetweenSettlementAndMaturity = (PHPExcel_Calculation_DateTime::_getDateValue(
                    $maturity
                ) - PHPExcel_Calculation_DateTime::_getDateValue($settlement));
        }
     ...
     * @param          array of float    $values        A series of cash flows that corresponds to a schedule of
     *                          payments in dates. The first payment is optional and corresponds to a cost or payment
     *                          that occurs at the beginning of the investment. If the first value is a cost or
     *                          payment, it must be a negative value. All succeeding payments are discounted based on a
     *                          365-day year. The series of values must contain at least one positive value and one
     *                          negative value.
     * @param          array of mixed    $dates        A schedule of payment dates that corresponds to the cash flow
     *                          payments. The first payment date indicates the beginning of the schedule of payments.
